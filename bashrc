...
# swap function - swaps two files/ folders with a temp file
function swap() {
    local TMPFILE=tmp.$$
    mv "$1" $TMPFILE && mv "$2" "$1" && mv $TMPFILE $2 && echo "Swapped!";
    [[ $? -ne 0 ]] && echo "Failed to swap."
}
